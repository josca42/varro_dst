...
notes:
- Monthly version of skib31 — same faerge codes and enhed selector, but monthly data. Prefer skib33 for current/recent data or when monthly resolution is needed.
- enhed is a measurement selector with 20 values — always filter to exactly one. Do not sum across enhed.
- faerge=10002 (FÆRGERUTER I ALT) is the pre-computed total. Exclude when summing individual routes.
- Domestic ferry routes only. For international monthly data use skib34.
- For long annual series back to 1990 use skib31.

table: fact.nabp36
description: 1-2.1.1 Produktion, BVT og indkomstdannelse (36a2-gruppering) efter transaktion, branche, prisenhed og tid
...
notes:
- branche join: `JOIN dim.nr_branche d ON d.kode = substring(f.branche from 2) AND d.niveau = 3`. Strip V prefix — niveau-3 codes have no hyphens. 38 sectors + V + VMEMO.

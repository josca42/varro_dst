...
- Contains 2 extra branche codes not in nabp10 and not in dim.nr_branche: TXB (banking/financial adjustment) and TXMEMO. These are needed to fully balance the accounting identity to V (total). For standard sector analysis, use nabp10 and exclude TXB/TXMEMO/VMEMO.
- transakt contains accounting aggregates — never sum across transakt.
- prisenhed must be filtered.
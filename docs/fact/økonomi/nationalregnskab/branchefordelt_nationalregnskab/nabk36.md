table: fact.nabk36
description: Akkumulations- og statuskonti, investering og beholdning af faste aktiver (36a2-gruppering) efter beholdning / strøm, aktiv, branche, prisenhed og tid
...
notes:
- branche join: `JOIN dim.nr_branche d ON d.kode = substring(f.branche from 2) AND d.niveau = 3`. Strip V prefix only — niveau-3 codes have no hyphens. 38 sectors + V + VMEMO.
- behold controls what indhold measures — never sum across behold. Mix of stocks and flows.

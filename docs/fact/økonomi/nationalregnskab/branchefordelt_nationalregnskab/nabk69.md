...
notes:
- branche join: `JOIN dim.nr_branche d ON d.kode = substring(f.branche from 2) AND d.niveau = 4`. Strip V prefix only — niveau-4 codes are 5-digit numeric. 69 sectors + V + VMEMO.

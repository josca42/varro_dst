table: fact.nabp69
description: 1-2.1.1 Produktion, BVT og indkomstdannelse (69-gruppering) efter transaktion, branche, prisenhed og tid
...
notes:
- branche join: `JOIN dim.nr_branche d ON d.kode = substring(f.branche from 2) AND d.niveau = 4`. Strip V prefix — niveau-4 codes are 5-digit numeric. 69 sectors + V + VMEMO.
- transakt contains accounting aggregates — never sum across transakt. Filter to one (typically B1GD for GVA).
- prisenhed must be filtered. Some transakt values only available in V (current prices).
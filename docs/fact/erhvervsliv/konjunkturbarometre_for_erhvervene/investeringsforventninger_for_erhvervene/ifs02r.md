...
notes:
- opg multiplies rows 4× per year — always filter to one opg value. Use opg=45 for final figures. Prefer ifs02 if revision tracking is not needed.
...
- invest: TOTAL is reported independently. Filter invest='TOTAL' for aggregate view; compare MASK/LAND/IMMA for composition analysis.

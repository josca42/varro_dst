...
notes:
- `tidspunkter` (11=Kalenderår, 22=Skoleår) and `overnat1` (11=Kursister, 13=Årselever) are both measurement selectors — always filter both to avoid doubling. Both are smallint.
- `fuddomr` hierarchy is identical to veuhoj11: TOT=H15, H1510=H151010, H1520=H152010 in practice. Use only leaf codes for breakdowns. veuhoj21 does NOT have H15201020 (Husholdningskursus, andre) — only veuhoj11 does.
- `alder` total code is `'0'`. `insttyp` is smallint — use `insttyp=0` for total.
- This table covers korte højskolekurser; veuhoj11 covers lange. Both share the same fuddomr coding scheme and structure. veuhoj11 goes back to 2005; this table only to 2011.